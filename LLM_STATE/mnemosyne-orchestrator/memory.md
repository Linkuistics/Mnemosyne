...
### System-wide daemon singleton lock, not per-vault
Lock file at `<runtime_dir>/mnemosyne/daemon.lock` (`$XDG_RUNTIME_DIR` on Linux, `$TMPDIR` on macOS) via `flock(2)`. Contains PID + vault path. Not inside the vault — invisible to Obsidian. Enforces v1 single-daemon-per-machine constraint. `daemon.pid` eliminated (redundant — lock file carries PID + vault path). Bootstrap subcommands read the lock file to determine whether a daemon is serving their target vault. Kernel-enforced crash safety — no stale-lock cleanup needed.

### Content-hash freshness checking via `Mnemosyne.FileIO`
All daemon file reads go through `FileIO.read_tracked/1` (records SHA-256 hash). All writes go through `FileIO.write_safe/3` with a file category: `:exclusive` (no check, overwrite — plan-catalog, runtime), `:phase_owned` (daemon-authoritative, warn on conflict — plan files during active phase), `:human_editable` (human-authoritative, discard on conflict — knowledge, routing.ex, expert declarations). Atomic temp-then-rename for all writes. Sub-B's `copy_back/2`, sub-E's `SafeFileWriter`, sub-F's plan-catalog, and sub-N's expert absorb all delegate to this module. `FreshnessTracker` GenServer owns an ETS table keyed by absolute path.

### No autonomous daemon git commits in v1
All daemon-triggered git operations are human-mediated. `Mnemosyne.VaultGit.commit/3` is the sole git interface. Vault git concurrency is a non-issue by design constraint. Sub-P (v2+) revisits with coordination-aware executor.
...
Three linked disciplines that compound: (1) **code-shaped contracts at brainstorm exit** — struct names, function specs, verdict atoms that let downstream amendment authors work mechanically rather than reason about intent; (2) **cross-sub-project requirements per-sibling** — one named section per affected sibling so each can find its own commitments without reading the whole spec; (3) **adoption stubs pushed to sibling backlogs before exiting** — brainstorm fresh context is the right moment to identify what each sibling must absorb; triage is too late.
...
|----|-------------|-----------|--------|-------|
| A  | Global knowledge store at user-specified path | Small-med | **done** (F+D amendments absorbed) | Design doc `specs/2026-04-13-sub-A-global-store-design.md`. Explicit vault discovery, `mnemosyne.toml` marker, `init`/`init --from`/`config use-vault`/`adopt-project` commands. F amendment absorbed via inline rewrite: §A1–§A10 re-cast for the Elixir daemon, §A4 layout picks up sub-F surfaces (`daemon.toml`, `routing.ex`, `plan-catalog.md`, `experts/`, `runtime/{daemon.sock,mailboxes/}`), `project-root/` rename absorbed in §A10 walk-up. D amendment absorbed: `daemon.lock` and `daemon.pid` removed from vault layout (singleton lock is system-wide), boot sequence collapsed from 11 to 10 steps. Q1–Q5 preserved verbatim with correction notes; Q6 (BEAM pivot) and Q7 (sub-F commitments) added in Appendix A. |
...
| D  | Daemon coordination (scope collapsed by F) | Small | **done** | Design doc `specs/2026-04-16-sub-D-coordination-design.md`. System-wide singleton lock at `<runtime_dir>/mnemosyne/daemon.lock` via `flock(2)` (not in vault — invisible to Obsidian). Content-hash freshness checking + `Mnemosyne.FileIO` as mandatory I/O layer with three file categories (`:exclusive`, `:phase_owned`, `:human_editable`). Vault git concurrency deferred (no autonomous commits in v1). `daemon.pid` eliminated (lock file carries PID + vault path). Sub-A design doc updated inline. Sibling plan at `LLM_STATE/sub-D-coordination/` with 7 tasks. |
| E  | Post-session knowledge ingestion | Medium | **done** (F amendment absorbed) | Design doc `specs/2026-04-12-sub-E-ingestion-design.md`. §1–§5 + Decision Trail rewritten inline: BEAM/Elixir, Stage 5 dispatch-to-experts; sub-E emits directly to sub-M’s bus from Stage 5 (no standalone `IngestionEvent` channel). Downstream task list still carries pre-pivot Rust framing; Task 0 gate required before implementation. |
...
Brainstorms complete: A, B, C, E, F, M, N. Amendments absorbed: A, B, C, E, M. Immediate pipeline: sub-E task-list gate task + sub-B/sub-C task-list rewrites → sub-F Task 0 → remaining brainstorms D, G, H, I → sub-O (v1.5) → sub-Q (v1.5, research) → sub-R (v1.5, research) → sub-P (v2).

Critical next steps: **(1)** Sub-E task-list rewrite gate task — analog of sub-B/sub-C/sub-M Task 0; sub-E’s sibling-plan task list carries pre-pivot Rust framing and must be rewritten before implementation. **(2)** Sub-B and sub-C downstream task-list rewrites against their rewritten design docs (gate tasks in place in both sibling plans; two remaining gate conditions for sub-F’s Task 0). **(3)** Sub-F Task 0 readiness check once (2) completes — then Task 1+ implementation can begin. **(4)** Remaining brainstorm/amendment tasks for D, G, H, I in any order; all four produce scope-framing documents. Sub-M’s task-list rewrite is internal and not on F’s critical path. Sub-Q and sub-R reserved for v1.5+.
## Open questions
