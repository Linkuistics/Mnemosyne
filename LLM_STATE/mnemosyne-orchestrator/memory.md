...
When a pivot invalidates large portions of a design doc, rewrite §1-§N with fresh content. Do not append a "§X is authoritative where earlier sections conflict" amendment block — it forces readers to context-switch between incompatible framings and stale content drifts from reality. History belongs in the Decision Trail appendix, with corrections inline at the original question entry. Cross-sub-project requirements re-evaluate on pivot (drop dead requirements, restate survivors in new terms) rather than translate mechanically. Multiple amendments stack in a single rewrite pass — each gets its own Decision Trail entry (Qn+1, Qn+2, …) rather than being folded together. Validated at scale: sub-C at 1186 lines with one shift, sub-B at 2296 lines absorbing three simultaneous shifts (LLM_CONTEXT overhaul, sub-F actor commitment, BEAM pivot).
...
| F  | Plan hierarchy + actor model + dispatch + declarative routing | **Large** | **done** (sibling plan scaffolded Session 13) | Design doc `specs/2026-04-14-sub-F-hierarchy-design.md`. Committed: persistent BEAM daemon, two sealed actor types, two message types, `project-root` convention, path-based qualified IDs, dispatch asymmetry, vault catalog, Datalog-ish declarative routing with Level 2 fallback, Elixir runtime, Rust TUI client. Sibling plan at `LLM_STATE/sub-F-hierarchy/` with 28 implementation tasks + Task 0 readiness gate; implementation blocked on sub-B + sub-C task-list rewrites and sub-A + sub-M amendment absorption. |
...
Committed brainstorms: A, B, C, E, M, F → **BEAM spike done (pipes validated)** → **sub-C amendment absorbed (Session 11)** → **sub-B amendment absorbed (Session 12)** → **sub-F sibling plan scaffolded (Session 13, P3.1 done — 28 implementation tasks + Task 0 readiness gate)** → amendment tasks for A, E, M merge into their design docs via inline rewrite (B's rewritten §4 now provides concrete inputs for E and M) → remaining brainstorms D, G, H, I, N (with D dramatically collapsed) → sub-O (v1.5) → sub-P (v2).

Critical next steps: **(1)** Sub-N brainstorm (v1-critical, independently executable — ExpertActor must ship alongside PlanActor, and sub-F's Task 3 stub is now in place waiting for it); **(2)** remaining amendment tasks for A, E, M (concrete inputs from B's §4.2 ReflectExitHook, §4.4 typed events, and C's §11 contracts now available); **(3)** sub-B and sub-C downstream task-list rewrites against their rewritten design docs (gate tasks in place in both sibling plans, and they are also gate conditions for sub-F's Task 0); **(4)** sub-F's Task 0 readiness check once (2) and (3) complete — then Task 1+ implementation can begin.
