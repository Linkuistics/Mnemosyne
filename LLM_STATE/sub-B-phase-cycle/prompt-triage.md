# Triage Phase — Sub-project B: Phase Cycle Reimplementation in Rust
...
- **Priority changed?** Move tasks up or down in file order to reflect
  current priority. The work phase picks from the top by default. Pay
  special attention to the Obsidian symlinks validation spike — it is
  task 0 and blocks everything else until it passes.
- **Dependencies still right?** If implementation has revealed that a task
  ...
  cross-sub-project dependencies (A, C, D stubs → real implementations).
...
- Implementation surprises that require follow-up work (e.g., a substitution
