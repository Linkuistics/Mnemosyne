# Memory — Sub-project B: Phase Cycle Reimplementation in Rust
...
`{{PROJECT}}`, `{{PLAN}}`, and `{{PROMPTS}}` substituted to absolute paths
before the harness sees them. `copy_back` reverses the substitution on
write-through. This is the load-bearing v1 acceptance criterion from the
seed plan's memory.
...
16. **End-to-end dogfood test**: run the orchestrator seed plan on
    Mnemosyne-hosted phase cycles, retiring `run-backlog-plan.sh` for
    that plan. This is the v1 acceptance test.
